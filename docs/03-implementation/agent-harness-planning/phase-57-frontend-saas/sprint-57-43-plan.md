...
|------------|------------|
| Backend response schema vs mockup TENANT row schema mismatch (e.g. `created_at` ISO format vs mockup display; `agents` count not in response) | Day 0 Prong 3 Schema verify; field adapter in `AdminTenantsView`; if backend missing fields → AP-2 banner + Phase 58+ extension AD |
