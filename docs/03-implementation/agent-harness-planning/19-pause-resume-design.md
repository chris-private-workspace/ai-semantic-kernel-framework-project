...
- **Generalized pause points** — PARTIALLY SHIPPED (Slice 3 legs 1+2):
  ...
  - STILL DEFERRED (leg 3): **mid-thinking pause** (interrupt an in-flight streaming LLM call). An **output-guardrail ESCALATE pause** (the per-response `_cat9_output_check` ESCALATE → pause, distinct from the between-turns gate) is also a possible future leg (the primitive supports it).
