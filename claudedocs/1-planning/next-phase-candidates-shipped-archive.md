...
## 🆕 Sprint 57.167 Carryover — de-Potemkin: honest MOCK surface + honest dead Filter control (MERGED PR #397, main `c001306d`)

**SHIPPED → closes BOTH core Potemkins named by the 2026-06-26 reality audit** (the 2026-07-15 re-scan found them `原封不動 — 核心 Potemkin 2 → 2, 0 修` after ~20 sprints). User-selected 2026-07-23 ("先做零成本修復，兩個 Potemkin 一次清掉") with 2 AskUserQuestion scope picks: `log + [MOCK] 標記` and `誠實禁用 + tooltip`. **(1) Mock-as-real**: `business_domain_mode` defaults to `"mock"` and said so NOWHERE (grep `[MOCK]|is_mock|mock_marker` over `business_domain/` + `agent_harness/` = **0 hits**) → a NEW `_warn_business_domain_mock()` in the lifespan (mirrors the 5 `_wire_*` helpers; `mock` → `logger.warning` naming the 18 tools FABRICATED **and** `BUSINESS_DOMAIN_MODE=service` as the way out; `service` → `logger.info` positive confirmation so a *broken* warning is never indistinguishable from production) **plus** a `"_mock": true` marker on every mock-sourced result via `_mark_mock_result` (`functools.wraps`) applied by **KEY-DIFF** at the `register_all_business_tools` chokepoint (snapshot `set(handlers)` before the 5 `register_*_tools`, wrap only the delta → co-registered echo/sandbox/memory/todos never touched, asserted by object identity) and **only when `mode=="mock"`** (service wraps nothing → byte-identical, asserted). Chosen over editing the 5 `mock_executor.py` because the chokepoint already knows `mode` and owns all 18 handlers → a future service swap needs zero un-marking. **(2) Dead control**: chat-v2 `SessionList.tsx:159-166` rendered a `.btn.ghost` with **no `onClick`** → now `disabled` + `title={t("chat.session.filterUnavailable")}` (new symmetric en/zh-TW key), markup/classes/icon verbatim — disabled rather than wired because the mockup it was ported from (`page-chat.jsx:127`) **itself defines no filter behaviour**, so inventing one breaches 「自創 widget」 and deleting it breaches the widget-deletion ban. **Two Day-0 Prong-2 catches shaped the design**: all 5 domains `return json.dumps(...)` + 6+ integration sites `json.loads` the result → marking had to go INSIDE the JSON (a `[MOCK] ` prefix emits invalid JSON); and `inspect.signature` (`executor.py:320`) follows `__wrapped__` → `functools.wraps` is **load-bearing** for the 1-arg/2-arg dispatch (asserted through the REAL `ToolExecutorImpl` — a 2-arg handler still receives its `ExecutionContext`). Day-0 also surfaced, out of scope, that the 18 tools are named `mock_*` in **service** mode too (18/18) → in production the NAME mislabels REAL data. Backend 2 files + FE 1 + 2 i18n + 3 test files; NO migration / wire event / codegen / `agent_harness` runtime change; parent-direct `agent_factor` 1.0. **🔴 The drive-through caught a real bug all 9 green tests missed**: run 1 rendered **ZERO** markers — `mock_incident_list` returns a **bare JSON array**, the wrapper skipped arrays, and a *passing* test (`test_json_array_left_untouched`) had **certified the skip**; every `*_list`/`*_query` tool would have shipped unmarked on a fully green gate (mypy 400 + 12/12 lints + 9/9). Fixed by `_marked_payload()` marking every dict element of a list (array shape preserved, still `json.loads`-able); the misleading test replaced by one naming the drive-through as its provenance. **Drive-through PASS** (real chat-v2 + real Azure gpt-5.2 + `mock_services` :8001, default config): Leg 1 startup WARNING verbatim in the fresh log · Leg 2 **10 × `"_mock": true`** in the live ToolBlock output, one per incident object, all original fields intact · Leg 3 Filter `disabled` + `title="Session filtering is not available yet"`, and after the locale toggle `工作階段篩選功能尚未提供` / `aria-label="篩選"` — resolving live in BOTH locales. pytest **3278** (+13) · Vitest **939** (+2) · mypy `src` 400 · run_all 12/12 · mockup 51 diff-empty. NEW calib `de-potemkin-honest-surface` 0.85 (1st pt ~1.22-1.27 edge-OVER — the overshoot is the drive-through FINDING the bug, not mis-estimation; if a 2nd >1.20 same cause → 1.0). **NEW carryover** `AD-BusinessTool-Name-Says-Mock-In-Service-Mode` · `AD-BusinessMock-ChatV2-Badge` (answer-bubble badge; the marker today needs the Inspector open) · `AD-SessionList-Filter-Real-Implementation` (blocked on a mockup spec) · `AD-Day0-Probe-Value-Shape-Not-Just-Call-Shape` (retro Q4: verifying the CALL shape is not verifying the VALUE shape). CHANGE-136, no design note. Detail: `memory/project_phase57_167_de_potemkin_honest_surfaces.md`.

---

## 🆕 Sprint 57.166 Carryover — cross-burst turn/token aggregate in the final `loop_end` (engine-debt, task-primitive range; MERGED PR #395, main `a289fecf`)

**SHIPPED → closes `AD-Scheduler-Burst-Stats-Aggregate-Phase58` (57.157's own carryover).** The 57.157 scheduler runs the loop across N bursts, each emitting its OWN `LoopCompleted`; the intermediates are swallowed so the FE sees one terminal `loop_end` — which therefore reported **only the last burst**. A 7-turn / 3-burst run told the human "2 turns" and wrote the same understatement into the `conversation_completed` audit row (turns + total/input/output tokens). Fix = 4 surgical edits inside `_stream_loop_events` (`api/v1/chat/router.py`): 4 accumulators (`agg_turns/agg_total_tokens/agg_input_tokens/agg_output_tokens`) before the `async for` → `+=` on EVERY `LoopCompleted` (swallowed + final) before serialize → patch `payload["data"]["total_turns"] = agg_turns` on the FINAL frame only (`not _swallow`) → audit `operation_data` 4 counters read the aggregates. **Billing deliberately left per-burst** (`event.*`): Day-0 Prong-2 `D-billing-per-burst-ungated` proved `router.py:1024`'s enqueue is a standalone `if` NOT gated on `_swallow` (fires once per burst), so the obvious "mutate the event to carry the aggregate" design would have **double-billed** burst 1 inside burst 2's row — a ~5-min grep that prevented a revenue-correctness bug; T4 locks it. Keeping the aggregate in separate locals is what makes both read-sites right at once. `sse.py`'s `loop_end` carries `total_turns` but not `total_tokens` (Day-0 `D-loopend-payload-fields`) → the FE-visible aggregate is 1 field; the token aggregate lives in the audit row. Scheduler OFF / single burst → `agg == event.*` → byte-identical. Backend-only: 1 production file + 1 NEW test file; NO migration / NO wire or codegen change / NO frontend / NO `agent_harness/` change; parent-direct agent_factor 1.0 (3-segment). **Drive-through PASS** real chat-v2 + real backend + real Azure `gpt-5.2` (scheduler ON, `max_bursts=3`): **3 bursts** (3 × `loop_start` + 3 `agent_loop.run` spans), exactly ONE terminal `loop_end` `stop=end_turn turns=7`, ChatHeader badge **"· 7 turns"** == audit `total_turns` **7** — with the per-burst ceiling in force 7 is arithmetically impossible for a single burst, so the badge is provably the aggregate (pre-fix both surfaces would have read ≤ the last burst). Needed **two temporary user-approved levers**, both reverted with proof: `handler.py` `max_turns` 8→2 (`git diff` empty after) + `HITL_ENABLED=false` (env only; backend restarted with no flags). Two earlier legs logged **inconclusive**, NOT passes — (a) burst 2 froze at the `python_sandbox` HITL gate with 0 completed turns → 2+0=2 indistinguishable from last-burst-only, (b) real gpt-5.2 REFUSED to self-loop on a `write_todos`-only prompt ("I can't keep calling write_todos turn after turn on my own") so the ceiling was unreachable by persuasion. pytest: scheduler+router+new = 47 pass · new `test_scheduler_burst_stats.py` 4 pass (T1 audit 15/170/135/35 · T2 REAL-serialized wire 15 · T3 OFF 8 · T4 billing 80/20 then 55/15) · chat integration 27 pass · mypy `src` 400 · run_all 12/12. Calib `scheduler-cross-burst-spike` **RE-POINTED 0.60 → 0.70** (2nd pt ~1.9-2.1; code+tests held 0.60 BOTH points — the entire over-edge is drive-through determinism, so the 0.70 buys the LEVER not code hours). **NEW carryover** `AD-Scheduler-Burst-CacheRate-Aggregate` (a *rate* needs weighted re-derivation, not a sum) · `AD-Scheduler-AwaitingApproval-Burst-Turn-Count` (HITL-paused burst reports 0 turns) · `AD-DriveThrough-Determinism-Lever-Plan-At-Day0`. CHANGE-135, no design note. Detail: `memory/project_phase57_166_burst_stats_aggregate.md`.
