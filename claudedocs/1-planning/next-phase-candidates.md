...
## 📦 Shipped Sprints — Pointer Index (57.29 → 57.167)
...
|--------|-----------------------|----------------|
| 57.167 | **de-Potemkin**: honest MOCK surface + honest dead Filter control — closes BOTH core Potemkins from the 2026-06-26 reality audit (re-scan 7-15: 原封不動, 0 fixed in ~20 sprints). (1) startup `logger.warning` naming the 18 business tools FABRICATED + `"_mock": true` on every mock-sourced result (key-diff chokepoint wrapper, mode-gated; service byte-identical); (2) chat-v2 Filter button `disabled` + localized tooltip (mockup defines no filter behaviour → not wired, not deleted). **Drive-through found the marker non-functional for every `*_list` tool** (bare JSON array; a passing test certified the skip) → fixed → PASS. **MERGED PR #397, main `c001306d`** | memory/project_phase57_167_de_potemkin_honest_surfaces.md |
...
#### 57.167
**NEW carryover**: `AD-BusinessTool-Name-Says-Mock-In-Service-Mode` (Day-0 found the 18 business tools are named `mock_*` in **service** mode too, 18/18 — in production the NAME mislabels REAL data; a rename changes the LLM-visible tool contract → own sprint + drive-through) · `AD-BusinessMock-ChatV2-Badge` (surface mock mode on the answer bubble via a wire event + FE badge, so the marker doesn't require opening the Inspector — AskUserQuestion option C, declined as beyond zero-cost) · `AD-SessionList-Filter-Real-Implementation` (real session filtering; **blocked on a mockup spec** — the mockup defines no filter UI) · `AD-Day0-Probe-Value-Shape-Not-Just-Call-Shape` (codify retro Q4: a Day-0 probe verifying the CALL shape `return json.dumps(result)` is NOT verifying the VALUE shape `type(result)`; fold into `day0-plan-verify.md` Prong 2 if a 2nd sprint hits it).
