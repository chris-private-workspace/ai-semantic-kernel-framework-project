...
6. ~~#2 `AD-Eval-PassK-Reliability-Harness`~~ — ✅ **DONE Sprint 57.141** (offline `benchmark_pass_k.py`, 4 axes pass^k+consistency+λ+ε, hybrid oracle, 12-case corpus, 17 CI tests, ZERO src edit; drive-through real Azure 90 runs: pass^k=pass@1=100% (NO divergence — corpus too easy, valid verdict) BUT answer-consistency 75% + λ-degradation 50% surfaced — 2 pass@1-invisible signals; CHANGE-108 + design note 45).
7. **#5 `AD-Observability-OTel-GenAI-Schema`** — ← **next per the ranked order**; net-new; CNCF OTel GenAI semantic conventions vs the self-authored wrapper; independent of `loop.py`. (Or the user may pick #7 tool-desc lint.)
