...
where agent_factor = {
  ...
  agent-delegated (tier-4 sub-class table — Sprint 57.55 retro Q4 tier-4 SPLIT ACTIVATED effective 2026-05-28 onwards):
    ...
    mixed-multidomain-bundle-mechanical:          0.65   (3+ independent tracks WITH mechanical pattern reuse component — e.g. backend ORM + Pydantic + tests bundle; evidence: Sprint 57.46 ratio 1.60 at 0.45→rollback to 0.65 — UNCHANGED from Sprint 57.48 Option B; Sprint 57.58 tier-3 1st validation ratio ~0.49 BELOW band by 0.36 → single-data-point caution KEEP, flag Sprint 57.59+ 2nd validation: if also < 0.7 tighten 0.45, if > 1.20 rollback 1.0)
