...
| **`frontend-mockup-strict-rebuild` (NEW Sprint 57.23)** | **0.60** (HYBRID weighted blend per Sprint 57.22 AUDIT-REPORT §Sprint 57.23+ Recommendation: auth-flow IAM-ish × 0.55 ~25% + mockup-direct-port-structural × 0.85 ~30% + 4-step wizard × 0.50 ~15% + 6-digit input grid × 0.55 ~15% + AP-2 banner reuse × 0.40 ~15% = ~0.60 mid-band) | **57.23=0.59 (1)** | **n/a 1-data-point** | **NEW class baseline opens (closes Sprint 57.22 §Sprint 57.23+ Recommendation + Sprint 57.23 Q8 calibration);1st app ratio actual/committed 0.59 BELOW [0.85, 1.20] band by 0.26;ratio actual/bottom-up = 0.36 (bottom-up was 2.8× too generous, 0.60 haircut insufficient);reasons: mockup line-by-line port discipline removes design ambiguity + Sprint 57.18 tokens pre-aligned (Day 0 D-PRE-2 saved ~1-2 hr) + AP-2 banner + AuthShell footer pattern reused 3-7× each;KEEP 0.60 baseline per `When to adjust` 3-sprint window rule (1-data-point insufficient);if pattern recurs 2-3× propose 0.60 → 0.40-0.45 (mechanical-class lift parallel to Sprint 57.16 AD-Sprint-Plan-13);next data point likely Sprint 57.24+ if another mockup-direct rebuild bundle (Ops follow-on / Chat-v2 Phase-2 / Admin Console rebuild) lands |
