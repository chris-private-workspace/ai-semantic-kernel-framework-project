...
| **`frontend-verbatim-css-repoint` (NEW Sprint 57.29; baseline LIFTED 0.60→0.50 Sprint 57.31 Day 4)** | **0.50** (was 0.60 Sprint 57.29-57.31; lifted Day 4 per `When to adjust` after 57.30+57.31 both below band by 0.45+); HYBRID weighted blend unchanged | 57.29≈1.0 / 57.30≈0.40 / **57.31≈0.35 (3)** | 3-pt mean **≈0.58** lower edge of band; 2-of-3 below by 0.45+ | **3rd app (Sprint 57.31 /cost-dashboard) ratio actual/committed ≈0.30-0.43 BELOW [0.85, 1.20] band by 0.45-0.55. `AD-Sprint-Plan-frontend-verbatim-bimodal-watch` (57.30 carryover) RESOLVED — bimodal hypothesis WEAKENED + REJECTED: 57.29 + 57.31 are same rich-dashboard shape with vastly different ratios (1.0 vs 0.35), so shape is NOT the variance driver. Driver IS estimate generosity diminishing as class iteration matures (1st app had buffer for unknowns; 3rd app has minimal unknowns). Per `When to adjust` 3+ consecutive < 0.7 rule (57.30 + 57.31 + the 0.45+ below-band magnitude on 2 of 3 = clear signal) → LOWER baseline 0.60 → 0.50. NEW `AD-Sprint-Plan-frontend-verbatim-css-repoint-baseline-lift` validates 0.50 across next 2-3 sprints; if continues < 0.5 → consider 0.40 next iteration. Next data point likely Sprint 57.32+ on /sla-dashboard (validates rich-dashboard shape) OR /admin-tenants (NEW admin shape).** |
