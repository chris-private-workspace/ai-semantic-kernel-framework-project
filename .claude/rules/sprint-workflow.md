...
| **`frontend-mockup-direct-port` (NEW Sprint 57.20)** | **0.55** (HYBRID weighted blend: shell rewrite × 0.55 ~35% + per-page mockup-direct port × 0.50 ~50% + closeout × 0.80 ~15%) | **57.20=0.45-0.55 (1)** | **n/a 1-data-point** | **NEW class baseline opens (closes Sprint 57.20 retrospective Q2 calibration); 1st app ratio actual/committed 0.45-0.55 BELOW [0.85, 1.20] band by 0.30-0.40 (ratio actual/bottom-up = 0.25-0.30 → bottom-up 2× too generous); reason: Sprint 57.18 token foundation + Sprint 57.19 OverviewPage 1:1 port + Sprint 57.15-57.16 chat-v2 Tailwind migration front-loaded structural work, leaving Sprint 57.20 only with token-tree consistency layer + new shell wrapper; KEEP 0.55 baseline per `When to adjust` 3-sprint window rule; if pattern recurs 2-3× → AD-Sprint-Plan-NEW propose 0.55 → 0.35-0.40; if Sprint 57.21+ hits AD-ChatV2-Full-Mockup-Fidelity multi-sprint epic with genuine structural rewrite → ratio may swing back into band — reserve judgement** |
