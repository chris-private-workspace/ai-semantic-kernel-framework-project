 ...
 *   3. ThemeProvider toggles [data-theme] attr + html.dark class + persists to localStorage
 ...
 *   - 2026-05-22: Sprint 57.28 US-C2 — assert ThemeProvider drives [data-theme] attr
...
describe("ThemeProvider", () => {
  ...
  it("toggle persists theme + drives [data-theme] attr and html.dark class", () => {
    render(
      <ThemeProvider>
        <Toggler />
      </ThemeProvider>,
    ...
    // Sprint 57.28 US-C2: ThemeProvider drives the mockup [data-theme] attribute
    // (styles-mockup.css scopes its tokens by it) AND keeps the shadcn .dark
    // class in sync through the Phase 1 transition.
    expect(screen.getByText(/theme=dark/)).toBeInTheDocument();
    expect(document.documentElement.getAttribute("data-theme")).toBe("dark");
    expect(document.documentElement.classList.contains("dark")).toBe(true);
    expect(window.localStorage.getItem("ipa-theme")).toBe("dark");

    // Toggle once → light (user opt-out; mockup has a full light theme)
