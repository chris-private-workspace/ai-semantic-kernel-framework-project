...
| **Current Sprint** | Sprint 57.112 (PR-pending) — **IAM Block C MFA (TOTP-only vertical) — SHIPPED + drive-through ALL 3 legs PASS** (the next C-12 leg after invites/credentials/register/RBAC-JWT; closes the TOTP leg of `AD-Auth-MFA-Backend-IAM-Block-C`). `users` + `totp_secret`/`mfa_enabled` (migration 0029) + `TOTPService` (enroll/confirm/verify, mirrors the 57.86 `CredentialsService`) + `POST /api/v1/mfa/{enroll,enroll/confirm,verify}` + a password-login **challenge-token gate** (`mfa_enabled` → a short-lived `mfa_pending` `v2_mfa_challenge` cookie + `{mfa_required}` — NO session — and `/mfa/verify` swaps a valid TOTP for the real `v2_jwt` via the shared `auth.issue_session`); `/mfa/verify` is EXEMPT (exact-match) while `/mfa/enroll` stays full-session. Thin FE: password-login `mfa_required` branch + un-stubbed `/auth/mfa` (demo banner gone, honest WebAuthn 400, real error copy). TOTP secret stored base32 plaintext (no encryption util wired → deferred `AD-MFA-Secret-At-Rest-Encryption`). Gate: mypy 0/363 · run_all 10/10 (count 24) · full pytest 2546+5skip (+20, 0 del) · Vitest 840 (+3) · mockup 51. Drive-through (real UI :3007 + fresh backend PID 25896 + real Postgres, zero dev-login): enroll(API)→confirm→password-login(`mfa_required`)→TOTP→authenticated `/chat-v2`; wrong code → inline error. **D13 drive-through find FIXED**: the verify `fetchWithAuth` lacked `{redirectOn401:false}` → a wrong-code 401 bounced to SSO (Vitest mocks fetch + jsdom no-ops `window.location` → only the drive-through caught it). WebAuthn + recovery + OIDC-gate deferred. Detail: `memory/project_phase57_112_iam_mfa_totp.md` + CHANGE-079 + design note 30. Next: the carryover pool (`claudedocs/1-planning/next-phase-candidates.md` — MFA WebAuthn/recovery, Skills system, SOC 2 + SBOM, FE mockup rebuild). |
...
**Last Updated**: 2026-06-13 (Sprint 57.112 — IAM Block C MFA TOTP-only vertical SHIPPED, drive-through ALL 3 legs PASS; closes the TOTP leg of AD-Auth-MFA-Backend-IAM-Block-C; CHANGE-079 + design note 30); see `memory/` for sprint history
