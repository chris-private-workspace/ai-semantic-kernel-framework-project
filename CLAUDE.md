...
| **Current Sprint** | Sprint 57.120 (PR-pending, NOT pushed) — **chat-v2 Inspector Turn tab `active_skill` row — drive-through BOTH legs PASS** (closes the Inspector-panel leg of `AD-ChatV2-Inspector-Turn-Metadata-Wire` / ISSUE-5; penultimate Skills-epic item; 119✅→**120**→121; scope via AskUserQuestion: active-skill row 單列 over +model / token-sweep). 57.116 wired `loop_start.active_skill` onto the triggering UserTurn as a "⚡ {skill}" CHIP only; this surfaces the SAME skill as a per-turn Inspector ROW (alongside trace_id/tokens). **PURE FE + store** — the field is ALREADY on the wire (57.116) → NO new wire field / codegen / backend / migration (count 24). `AgentTurn += activeSkill?` + at `turn_start` carry it from the most-recent NON-injected UserTurn (the loop trigger; the same cross-slice point that links `span_id`; skip injected 57.101 → no clear-on-injection; a new no-skill loop → undefined → no stale leak) + a `<KV k="active_skill" v={⚡skill / "—"}>` in InspectorTurn (reuse the `KV` helper → no `styles-mockup.css` change → mockup-fidelity byte-identical). 5 files ALL FE, **0 `.py`** → mypy 0/371 / pytest 2648+5skip / wire 24 definitionally UNCHANGED. Gate: Vitest 884 (+5) · build ✅ (57.116 lesson: optional field, no literal break) · lint clean · mockup 51 · run_all 10/10. Drive-through (real chat-v2 + gpt-5.2, acme-prod, NO Risk-E restart — 0 `.py` delta): Leg A `/code-review`→user chip + Inspector Turn-1 `active_skill: ⚡ code-review`; Leg B plain msg→Inspector Turn-2 `active_skill: —` (new trace — prior skill did NOT leak = no-leak proven LIVE). AP-4 clear, 2 screenshots. NEW class `chatv2-inspector-existing-field-surface` 0.55→**0.85 re-pointed** (1st pt ~1.6 OVER — tiny-code+full-ceremony parent-direct sprint: the 0.45-0.65 band assumes code-hours to haircut; ceremony is NOT code-accelerated). Detail: `memory/project_phase57_120_chatv2_inspector_active_skill.md` + CHANGE-087 (NO design note — feature continuation). Next: `model` row + token sweep carried under same AD + last Skills item 121 slash-menu-mockup (⚠️ needs mockup first; `claudedocs/1-planning/next-phase-candidates.md`) + carryover pool. |
...
**Last Updated**: 2026-06-15 (Sprint 57.120 — chat-v2 Inspector Turn tab gains an `active_skill` KV row: carry the EXISTING `loop_start.active_skill` (57.116) forward onto `AgentTurn` at `turn_start` (from the most-recent non-injected UserTurn) + a `<KV k="active_skill" v={⚡skill / "—"}>` in InspectorTurn; PURE FE+store, 0 `.py`, NO new wire field/codegen/backend (count 24); closes the Inspector-panel leg of AD-ChatV2-Inspector-Turn-Metadata-Wire (ISSUE-5); drive-through BOTH legs PASS; CHANGE-087, NO design note (feature continuation)); see `memory/` for sprint history
