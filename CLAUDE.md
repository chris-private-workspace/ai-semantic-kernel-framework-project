...
| **Current Sprint** | **Sprint 57.167 — de-Potemkin: honest MOCK surface + honest dead Filter control — ⏳ PR-pending** (branch `feature/sprint-57-167-de-potemkin-honest-surfaces`). Closes **BOTH core Potemkins** named by the 2026-06-26 reality audit (7-15 re-scan: 原封不動, 0 fixed in ~20 sprints): (1) `business_domain_mode="mock"` announced nowhere → startup `logger.warning` + `"_mock": true` on every mock-sourced tool result (key-diff chokepoint wrapper, mode-gated; `service` byte-identical); (2) chat-v2 Filter button had no `onClick` → `disabled` + localized tooltip (mockup defines no filter behaviour → not wired, not deleted). **The drive-through found the marker non-functional for every `*_list` tool** (bare JSON array; a *passing* test had certified the skip) → fixed → PASS. CHANGE-136. Detail: `memory/project_phase57_167_de_potemkin_honest_surfaces.md`. (Prior: Sprint 57.166 MERGED PR #395, main `a289fecf`.) |
...
**Last Updated**: 2026-07-23 (Sprint 57.167 — de-Potemkin: honest MOCK surface + dead Filter control; ⏳ PR-pending); see `memory/` for sprint history
