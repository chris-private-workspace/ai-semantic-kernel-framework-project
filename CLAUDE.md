...
| **Current Sprint** | Sprint 57.101 (PR pending) — **B1 between-turns message injection primitive**: a chat-v2 user sends an instruction MID-RUN; the loop drains it at the next turn boundary (`MessageInbox` ABC + `_run_turns` drain seam) and the agent picks it up. One reusable primitive (**B2 TEAMMATE** reuses the same drain seam). **D-DAY1-1 (load-bearing)**: the injection is an INPUT → it runs the Cat 9 `check_input` (NOT the between-turns gate, which checks turn OUTPUTs via `_latest_output_text`); a non-PASS injection is DROPPED + `GuardrailTriggered(input)`, the run continues. Module-level `InjectionRegistry` (tenant-scoped) bridges the SEPARATE inject-POST + run requests (a per-request mailbox can't); `POST /{id}/inject` (404/409/202); new `MessageInjected` wire (a NEW event TYPE, count 23→24); FE composer usable mid-run (real_llm only, echo gated) → `inject()` + `message_injected`→`UserTurn(injected)` tag. mypy `src` 0/355 + full pytest **2320 +4 skip** (+20, 0 del) + run_all 10/10 + Vitest **787** (+5) + `check:mockup-fidelity` 53 unchanged. **Drive-through BOTH DoD cases PASS** (real UI + real Azure gpt-5.2, Playwright): Case A mid-run inject "check the checkout db pool" → the agent calls `mock_patrol_check_servers` + the summary has a "Checkout DB connection pool health:" section (injected UserTurn tag visible); Case B "approval required" inject → DROPPED + `guardrail_triggered input→escalate`, run continues. Infra incident (NOT B1): the Docker DB was down at session start → integration timeouts looked like a hang; the port check (`:5432 free`) was the tell. Detail: `memory/project_phase57_101_between_turns_injection.md` + CHANGE-068 + design note 26 + 17.md. Next candidates: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-06-11 (Sprint 57.101 — B1 between-turns message injection primitive: a `MessageInbox` ABC + `_run_turns` drain seam let a chat-v2 user inject an instruction MID-RUN; the agent picks it up at the next turn boundary; D-DAY1-1 = the injection is an INPUT so it runs `check_input` (a non-PASS injection is dropped + `GuardrailTriggered(input)`), not the between-turns gate which checks OUTPUTs; module-level `InjectionRegistry` + `POST /{id}/inject`; new `MessageInjected` wire 23→24; FE composer usable mid-run (real_llm) → `inject()` + `UserTurn(injected)`; full pytest 2320+4skip (+20); drive-through BOTH cases PASS; B2 TEAMMATE reuses the drain seam; CHANGE-068 + design note 26 + 17.md); see `memory/` for sprint history
