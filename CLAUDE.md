...
| **Current Sprint** | Sprint 57.85 (PR pending) — **C-12 IAM Block B: invites vertical spike** (closes `AD-Auth-Invite-Backend-IAM-Block-B-Phase58`; FIRST C-12 spike): NEW `invites` table (migration 0026, RLS two-policy + system-sentinel guest-lookup escape) + `InvitesService` (DB-opaque token sha256-stored-returned-once / create / get_metadata / single-use accept → User+UserRole+audit / revoke) + 3 endpoints (admin create `require_admin_platform_role` + guest GET/accept EXEMPT) + frontend invite page wired (remove fixture+AP-2 banner; 404/410 states). password accepted-not-stored (split → 57.86 `AD-Auth-Credentials-PasswordLogin-Phase58`). Spike → design note `21-iam-invites-spike.md` (8-pt gate). mypy 0/339 + pytest 2179 + run_all 10/10 + Vitest 757 + mockup-fidelity ✓; CHANGE-052. Detail: `memory/project_phase57_85_iam_invites.md`. Next: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-06-06 (Sprint 57.85 — C-12 IAM Block B invites vertical spike: invites table + migration 0026 (RLS two-policy + system-sentinel guest-lookup escape) + InvitesService (DB-opaque token / create / get_metadata / single-use accept → User+role+audit / revoke) + 3 endpoints (admin create + guest GET/accept exempt) + frontend invite page wired; password accepted-not-stored split → 57.86; closes AD-Auth-Invite-Backend-IAM-Block-B-Phase58; first C-12 spike; design note 21-iam-invites-spike.md 8-pt gate; mypy 0/339 + pytest 2179 + run_all 10/10 + Vitest 757 + mockup-fidelity ✓; CHANGE-052; PR pending); see `memory/` for sprint history
