...
| **Current Sprint** | Sprint 57.122 (PR-pending, NOT pushed) — **HITL policy read-side load-bearing — closes `AD-HITL-Policy-ReadSide-Potemkin-Phase58`, the FLAGSHIP "C. 主流量 Potemkin / 載重 gap" item** (the user's pick after the Skills epic). The per-tenant `HITLPolicy` risk thresholds (write-side shipped, never read at tool execution — the loop hardcoded `if requires_approval: ESCALATE` + a flat `RiskLevel.HIGH`) are now load-bearing: the loop's Cat 9 path reads `hitl_manager.get_policy(tenant_id)` + applies a pure two-threshold `decide_tool_hitl` (risk from `ToolSpec.risk_level`) + carries the resolved risk into the `ApprovalRequest`. Backend-only (2 src + 2 tests + 1 stale-assertion fix); mypy 0/371 · run_all 10/10 · pytest 2695+5skip. Drive-through PASS (real chat-v2 + gpt-5.2): the SAME `python_sandbox` call auto-approves vs escalates per the per-tenant DB policy set via admin PUT. Detail: `memory/project_phase57_122_hitl_policy_readside.md` + CHANGE-089 + design note 35. Next: remaining C-class Potemkin (`AD-FE-Tenant-Display-Fixture-Phase58`) + the carryover pool (`claudedocs/1-planning/next-phase-candidates.md`). |
...
**Last Updated**: 2026-06-15 (Sprint 57.122 — HITL policy read-side load-bearing: the loop's Cat 9 tool path now reads `hitl_manager.get_policy(tenant_id)` + applies a pure two-threshold `decide_tool_hitl` (risk from `ToolSpec.risk_level`) so per-tenant `HITLPolicy` thresholds finally decide escalate-vs-auto-approve, and escalations carry the resolved risk (hardcoded `RiskLevel.HIGH` dropped); closes `AD-HITL-Policy-ReadSide-Potemkin-Phase58` = the flagship "C. 主流量 Potemkin / 載重 gap"; backend-only, NO migration/wire/codegen/frontend; drive-through PASS (same `python_sandbox` call auto-approves vs escalates per per-tenant policy); CHANGE-089 + design note 35); see `memory/` for sprint history
