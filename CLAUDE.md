...
| **Current Sprint** | **Sprint 57.162 — DAG soft-enforce advisory + cycle report + Inspector Viz — committed LOCAL, PR-pending (not pushed per user 完成即暫停)** (branch `feature/sprint-57-162-dag-enforce-cyclereport-viz` from HEAD `1819180d`; engine-debt program, task-primitive range). Closes the 3 remaining 57.156 DAG carryovers (`AD-TaskPrimitive-DAG-{Enforce,CycleReport,Viz}-Phase58`) as one guidance-preserving slice: **Enforce=SOFT** (AskUserQuestion pick — `write_todos` appends a non-blocking ⚠️ advisory on out-of-order status, write still succeeds; hard-reject rejected as contradicting the deliberate guidance-not-enforcement principle), shared `detect_cycles`/`plan_warnings` surface cycles in the observation + `render_active_plan` `(blocked by cycle:…)`, FE `TodoDagGraph` leveled node-link Viz (`var(--*)` only, styles-mockup.css byte-identical). Serde-transparent — **NO migration/wire(26)/codegen/loop.py/sse.py** (backend 2 files + FE 1; wire already carries `depends_on` since 57.156). **Drive-through STRONG PASS** real chat-v2 + Azure gpt-5.2: advisory (out-of-order + cycle) in the real observation w/ the write succeeding + DAG graph (dashed-red cycle nodes) rendered live. pytest 3223 (+17) · Vitest 930 (+3) · mypy `src` 400 · run_all 11/11 · mockup 51. NEW calib `task-primitive-dag-spike` 0.60 2nd pt (~1.0-1.1 IN band). CHANGE-129, no design note. **Carryover NONE** (user 不要產生其他工作). DAG arc (57.140→156→157→162) complete. (Prior: Sprint 57.161 structural compactor real token re-count MERGED PR #378, main `bef3bca4`.) Detail: `memory/project_phase57_162_dag_enforce_cyclereport_viz.md`. |
...
**Last Updated**: 2026-07-08 (Sprint 57.162 — DAG soft-enforce advisory + cycle report + Inspector Viz: closes the 3 remaining 57.156 DAG carryovers `AD-TaskPrimitive-DAG-{Enforce,CycleReport,Viz}-Phase58` as one guidance-preserving slice. Enforce=SOFT (AskUserQuestion pick — `write_todos` appends a non-blocking ⚠️ advisory on out-of-order status, write still succeeds; hard-reject rejected as contradicting guidance-not-enforcement); shared `detect_cycles`/`plan_warnings` surface cycles in the observation + `render_active_plan` `(blocked by cycle:…)`; FE `TodoDagGraph` leveled node-link Viz (`var(--*)` only). Serde-transparent — NO migration/wire(26)/codegen/loop.py/sse.py (backend 2 files + FE 1; wire already carries `depends_on` since 57.156). Drive-through STRONG PASS real chat-v2 + Azure gpt-5.2: advisory (out-of-order + cycle) in the real observation w/ the write succeeding + DAG graph (dashed-red cycle nodes) rendered live. pytest 3223 (+17) · mypy `src` 400 · run_all 11/11 · Vitest 930 (+3) · mockup 51. NEW calib `task-primitive-dag-spike` 0.60 2nd pt (~1.0-1.1 IN band). CHANGE-129, no design note. Carryover NONE (user 不要產生其他工作). **committed LOCAL, PR-pending — NOT pushed per user 完成即暫停** (branch from HEAD `1819180d`); prior Sprint 57.161 MERGED PR #378 main `bef3bca4`; see `memory/` for sprint history
