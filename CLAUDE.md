...
| **Current Sprint** | Sprint 57.87 (PR pending) — **C-12 IAM Block B: self-service tenant registration backend** (closes `AD-Auth-Register-Backend-IAM-Block-B-Phase58`; 3rd C-12 spike): NEW `RegistrationService` (slug→409 / `Tenant` ACTIVE / **codebase's first real admin `Role`-creation** / `User`+`UserRole` / audit) + public EXEMPT `POST /tenants/register` + un-stubbed `/auth/register` wizard. Honest boundary: seeded admin role not yet JWT-authz-effective (`AD-RBAC-DB-To-JWT-Wiring`) + register-user↔OIDC-user linkage deferred. No migration / no mockup-CSS change. Spike → design note `23-iam-registration-spike.md` (8-pt gate). mypy 0/344 + pytest 2214 + run_all 10/10 + Vitest 763 + mockup-fidelity ✓ (oklch baseline 53); CHANGE-055. Detail: `memory/project_phase57_87_iam_registration.md`. Next: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-06-06 (Sprint 57.87 — C-12 IAM Block B self-service tenant registration backend: NEW RegistrationService (slug→409 / Tenant ACTIVE / first real admin Role-creation / User+UserRole / audit) + public EXEMPT POST /tenants/register + un-stubbed /auth/register wizard; closes AD-Auth-Register-Backend-IAM-Block-B-Phase58; 3rd C-12 spike; honest boundary: admin role not yet JWT-authz-effective + OIDC-user linkage deferred; no migration / no mockup-CSS change; design note 23-iam-registration-spike.md 8-pt gate; mypy 0/344 + pytest 2214 + run_all 10/10 + Vitest 763 + mockup-fidelity ✓ (oklch baseline 53); CHANGE-055; PR pending); see `memory/` for sprint history
