...
| **Current Sprint** | **No active sprint** (awaiting next-phase selection) — last shipped **Sprint 57.144 MERGED** (PR #341, main `06d2ca59`; branch from main `74555317`) — **Cat 2 tool-description lint + structured-error reflection** — closes `AD-Tool-Description-Lint-Reflection` (research #7, the **LAST canonical research item**; #6/#3/#8/#4/#1/#2/#5 all CLOSED). **Half A**: `check_tool_descriptions.py` AST lint (11th in run_all) — surfaced + filled **40 real missing param descriptions** (9 harness + 31 business_domain). **Half B** (evidence-first, default OFF): pure `classify_tool_error`/`render_reflection` (PARAMETER/WRONG_TOOL/FAILED_API/INVOCATION/UNKNOWN, orthogonal to Cat 8 ErrorClass) enriched at the executor (dominant path, fixes a latent empty-observation gap) + `loop.py:3023` rare path (B2), gated by `CHAT_TOOL_ERROR_REFLECTION`; `ToolResult += error_taxonomy`. **A/B (real Azure, 8 cases)**: fix_rate 87.5%=87.5% (+0.00% < 5% materiality) → **KEEP lever OFF** (strong model already recovers; consistent with §2.4 RL-confound hedge; reflection −32 tokens, opt-in lever shipped). pytest 2930+5skip · mypy 0/382 · run_all 11/11 · NO migration / wire 26 / NO frontend. NEW calib class `tool-reflection-and-lint-spike` 0.60 (1st pt ~1.05-1.1 IN band, parent-direct). CHANGE-111 + design note 48. (Prior: Sprint 57.143 user-Stop resume MERGED PR #339.) Detail: `memory/project_phase57_144_tool_error_reflection.md`. |
...
**Last Updated**: 2026-06-26 (Sprint 57.144 — Cat 2 tool-description lint + structured-error reflection; closes `AD-Tool-Description-Lint-Reflection` (research #7, LAST canonical item); Half A lint + 40 param fixes, Half B taxonomy + executor/loop enrich + env lever; A/B real Azure → KEEP lever OFF; CHANGE-111 + design note 48); see `memory/` for sprint history
