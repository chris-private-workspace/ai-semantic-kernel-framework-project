...
| **Current Sprint** | Sprint 57.71 (PR pending) — **A-4 Loop Tracer: real tracer injection (Tier 0) + loop-internal span tree (Tier 1)** — closes `AD-Cat12-LoopTracer` (Area-A item 4 of the A+B+C integration-gap analysis). Tier 0: `build_real_llm_handler`/`build_handler` gain a `tracer` param; `router.py` threads its real `OTelTracer` into the loop (was NoOp on the chat path). Tier 1: root span bound `as child` (D1 nesting fix) + `loop.py` opens `LOOP→TURN→{LLM_CALL, TOOL_EXEC, PROMPT_BUILD, COMPACTION}`; span-type via span name + `attributes["span_type"]` (SpanCategory enum unchanged — recorded `02.md §Naming Drift Note`); loop is the single trace-tree owner (adapter/executor/parser tracers stay NoOp — no double-instrumentation); span token/cost = attribute only (no ledger write). **VERIFICATION + MEMORY_OP deferred (N/A at loop level)** — verification is the outer `correction_loop.py` wrapper, memory is inside PromptBuilder/tools → "full span set" delivered as 6 span types + 2 documented deferrals (AP-4 avoided). Feature-continuation (no design note). pytest 2049→2057 (+8) / mypy src 0/329 / 10/10 V2 lints / black 605 unchanged / no FE·wire·codegen·migration. Detail: `memory/project_phase57_71_loop_tracer.md`. Next candidates: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-06-02 (Sprint 57.71 — A-4 Loop Tracer (Tier 0 real tracer injection + Tier 1 loop-internal span tree): closes `AD-Cat12-LoopTracer`; `build_real_llm_handler`/`build_handler`/`router.py` thread a real `OTelTracer` into the loop (was NoOp); `loop.py` binds the root span `as child` (D1 nesting fix) + opens `LOOP→TURN→{LLM_CALL, TOOL_EXEC, PROMPT_BUILD, COMPACTION}` spans, span-type via name + `attributes["span_type"]` (SpanCategory enum unchanged), loop = single trace-tree owner (no double-instrumentation), span cost = attribute only (no ledger write); VERIFICATION + MEMORY_OP deferred (N/A at loop level — verification is the outer wrapper, memory is inside PromptBuilder/tools); feature-continuation (no design note); pytest 2049→2057 (+8) / mypy src 0/329 / 10/10 V2 lints / black 605 unchanged / no FE·wire·codegen·migration; CHANGE-039; PR pending); see `memory/` for sprint history
