...
| **Current Sprint** | Sprint 57.60 closed 2026-05-29 — **RateLimits MetaData Cleanup** closes `AD-RateLimits-MetaData-Cleanup-Phase58`: retire transitional `meta_data["rate_limits"]` fallback at 4 read sites + PUT dual-write + Alembic `0020` strip JSONB + reverse-populate downgrade (config single-source `rate_limit_configs`; Phase 58.x RateLimits arc config-complete 57.58+57.59+57.60). API shapes UNCHANGED → frontend untouched. single agent 26th consecutive; 2 PROMOTIONS codified (Prong 2 nested-shape-read + Prong 3 physical-column-vs-ORM-alias). Calibration: `mechanical-pattern-reuse-heavy` 0.30 1st forward application ~1.09 IN BAND KEEP; `medium-backend` 0.80 11th data point ~0.33 confound-resolved KEEP. DUAL CLEAN 22/22 PARITY **16 consecutive 57.45-57.60**. Detail: `memory/project_phase57_60_rate_limits_metadata_cleanup.md`. Next candidates: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-05-29 (Sprint 57.60 — RateLimits MetaData Cleanup; retire transitional `meta_data["rate_limits"]` fallback at 4 read sites + PUT dual-write + Alembic `0020` strip JSONB + reverse-populate downgrade **closes `AD-RateLimits-MetaData-Cleanup-Phase58`**; config single-source `rate_limit_configs` (Phase 58.x RateLimits arc config-complete 57.58+57.59+57.60); API-shapes-unchanged frontend untouched; 2 PROMOTIONS codified (Prong 2 nested-shape + Prong 3 physical-column-vs-ORM-alias); `mechanical-pattern-reuse-heavy` 0.30 1st forward ~1.09 IN BAND KEEP; pytest 1840→1848; DUAL CLEAN 16 consec 57.45-57.60; commits `621afe72`+`416c9f84` Day 0+1 + Day 2 pending); see `memory/` for sprint history
