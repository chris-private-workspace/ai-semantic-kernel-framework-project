...
| **Current Sprint** | **Sprint 57.149 — memory-formation Slice 2 (Option-B deterministic post-send extraction) code-complete, PR-pending** (branch `feature/sprint-57-149-memory-auto-extract` from main `694e0062`; NOT yet pushed — awaiting user confirmation). Wires the long-unwired `MemoryExtractor` (Cat 3, since 51.2) onto the chat path: after EVERY real_llm send (env-gated `CHAT_MEMORY_AUTO_EXTRACT` default ON, cheap tier), a Starlette **`BackgroundTask`** deterministically extracts durable user facts → `UserLayer` tagged `source="auto_extract"`, so the platform forms memory EVEN WHEN the agent never calls `memory_write` (57.148 Option-A nudge is discretionary; this is the deterministic safety net). Prompt-level dedup reads the 57.148 `profile()` first. Backend-only, NO migration (the `memory_user.source` column pre-existed)/wire(26)/frontend; `CHAT_MEMORY_AUTO_EXTRACT=false` or `user_id=None` byte-identical. **Drive-through PASS** (real chat-v2 + real Azure gpt-5.2, 3 legs): dan states "Project Aurora, Oracle→PostgreSQL, Q3" → `[auto_extract]` conf 0.99 row → NEW session recalls Aurora (via always-on `profile()`, Inspector read of `memory_user:5d487f86`) → priya (diff user) "我不知道你是誰" (per-user isolation). Day-3 evolution: extraction moved off the SSE generator to a BackgroundTask (connection-close no longer blocked; the GeneratorExit/OTel noise was found PRE-EXISTING `tracer.py:200`, NOT the extraction). pytest 3013 (+14) · mypy 0/392 · v2 lints 25 · FE untouched. NEW calib `memory-formation-extract-spike` 0.60 → re-pointed **0.85** (1st pt ~1.3-1.4 OVER per 57.148 note; drive-through discovery loop = variance driver). CHANGE-116 + design note 53. Closes `AD-Memory-Formation-Auto-Extract` (upsert-by-key / 缺口-2 session-recall / semantic axis / NEW judge-memory-blind open). (Prior: Sprint 57.148 MERGED PR #350, main `4fe02965`.) Detail: `memory/project_phase57_149_memory_auto_extract.md`. |
...
**Last Updated**: 2026-06-28 (Sprint 57.149 — memory-formation Slice 2: Option-B deterministic post-send extraction — wire `MemoryExtractor` → cheap-tier Starlette `BackgroundTask` after every send → `UserLayer` tagged `source="auto_extract"`; forms memory even when the agent never calls `memory_write`; prompt-level dedup via 57.148 `profile()`; drive-through PASS — Aurora formation + cross-session recall + per-user isolation on real chat-v2 + Azure gpt-5.2; CHANGE-116 + design note 53; code-complete PR-pending main `694e0062`); see `memory/` for sprint history
